...
public class Paint2 extends Window {
    ...
    protected void paintComponent(Graphics g) {
        ...
        if (thePic.size() > 0) {
            ...
            theSplinePath = new Path();
            spline(g, ax, ay, splineMid.x, splineMid.y, cx, cy, 5);
        }
        theSplinePath.draw(g); // Must have this to show the spline while moving the mouse.
    }

    public static void spline(Graphics g, int ax, int ay, int bx, int by, int cx, int cy, int n) {
        if (n == 0) {
            // No longer draw but just add to the path.
            theSplinePath.add(new Point(ax, ay));
            theSplinePath.add(new Point(cx, cy));
            return;
        }
    ...
    public void mousePressed(MouseEvent me) {
        ...
        thePic.add(theSplinePath); // Must add this to make the spline stop and painted on the window
        repaint();
    }
